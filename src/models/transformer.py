...
class CrossAttentionBridge(nn.Module):
    ...
    def forward(self, cls_token: torch.Tensor, other_sequences: torch.Tensor, key_padding_mask: torch.Tensor) -> torch.Tensor:
...
class ASRModelSelector(nn.Module):
    def __init__(self, model_dims: dict[str, int], model_names: list[str], d_model: int = 256, n_heads: int = 4, stage1_layers: int = 2, stage2_layers: int = 1, ffn_dim: int = 512, dropout: float = 0.1, use_cross_attention_bridge: bool = True, share_stage1_weights: bool = True, max_seq_len: int = 2500):
        super().__init__()
        ...
        self.pos_encoder = SinusoidalPositionalEncoding(d_model, max_len=max_seq_len, dropout=dropout)
        ...
        if share_stage1_weights:
            encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=n_heads, dim_feedforward=ffn_dim, dropout=dropout, batch_first=True, activation="gelu")
            self.stage1_encoder = nn.TransformerEncoder(encoder_layer, num_layers=stage1_layers)
        else:
            self.stage1_encoders = nn.ModuleDict()
            for name in model_names:
                encoder_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=n_heads, dim_feedforward=ffn_dim, dropout=dropout, batch_first=True, activation="gelu")
                self.stage1_encoders[name] = nn.TransformerEncoder(encoder_layer, num_layers=stage1_layers)
        ...
        fusion_layer = nn.TransformerEncoderLayer(d_model=d_model, nhead=n_heads, dim_feedforward=ffn_dim, dropout=dropout, batch_first=True, activation="gelu")
    ...
    def _run_stage1(self, model_idx: int, model_name: str, hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    def forward(self, hidden_states: dict[str, torch.Tensor], attention_masks: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        cls_outputs, full_sequences, full_padding_masks = {}, {}, {}

        for idx, model_name in enumerate(self.model_names):
            cls_out, full_seq, pad_mask = self._run_stage1(idx, model_name, hidden_states[model_name], attention_masks[model_name])
        ...
        if self.use_cross_attention_bridge:
            ...
            for model_name in self.model_names:
                ...
                other_seqs = torch.cat([full_sequences[n][:, 1:, :] for n in other_names], dim=1)
                other_masks = torch.cat([full_padding_masks[n][:, 1:] for n in other_names], dim=1)
                updated_cls[model_name] = self.cross_attention_bridges[model_name](
                    cls_token=cls_outputs[model_name], other_sequences=other_seqs, key_padding_mask=other_masks
                )
...
class ASRModelSelectorLightning(BaseASRSelectorLightningModule):
    """Lightning Module wrapper for the Hierarchical Transformer architecture."""

    def __init__(self, model_dims: dict[str, int], model_names: list[str], d_model: int = 256, n_heads: int = 4, stage1_layers: int = 2, stage2_layers: int = 1, ffn_dim: int = 512, dropout: float = 0.1, use_cross_attention_bridge: bool = True, share_stage1_weights: bool = True, max_seq_len: int = 2500, optimizer_cfg=None, scheduler_cfg=None):
